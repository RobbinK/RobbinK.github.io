...
final class xmlutils
{
        ...
        *
        * @param array $data
        * @param string $ROOT_PATHNodeName - what you want the ROOT_PATH node to be - defaultsto data.
        * @param SimpleXMLElement $xml - should only be used recursively
        * @return string XML
        */
        public static function array_to_xml($data, $ROOT_PATHNodeName = 'data', $xml=null, $parentXml=null)
        {
                ...
                //if ($ROOT_PATHNodeName == false) {
                //      $xml = simplexml_load_string("<s/>");
                //}
                if ($xml == null)
                {
                       $xml = simplexml_load_string("<?xml version='1.0' encoding='utf-8'?><$ROOT_PATHNodeName />");
                }
                ...
                foreach($data as $key => $value)
                {
                        ...
                        $nodeName = is_numeric($key) ? $ROOT_PATHNodeName . '-item' : $key;
                        ...
                        if (is_array($value)) {
                                ...
                                if (is_numeric($key)) {
                                        // Another exception if this is teh ROOT_PATH node and is an array.  In this case we don't have a parent node to use so we must use the current node and not update the reference.
                                        if($parentXml == null) {
