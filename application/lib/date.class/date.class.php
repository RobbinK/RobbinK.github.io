...
//	getdate()	mixed		return specific part of the date class
...
class DateClass
{
	...
	//=========================================================
	function getdate($datePart="",$dateTime="")
	{
	...
	{
		if ($dateTime) $this->SetDate($dateTime);
		return $this->getdate("mon");
	}
	function Day($dateTime="")
	{
		if ($dateTime) $this->SetDate($dateTime);
		return $this->getdate("mday");
	}
	function Hours($dateTime="")
	{
		if ($dateTime) $this->SetDate($dateTime);
		return $this->getdate("hours");
	}
	function Minutes($dateTime="")
	{
		if ($dateTime) $this->SetDate($dateTime);
		return $this->getdate("minutes");
	}
	function Seconds($dateTime="")
	{
		if ($dateTime) $this->SetDate($dateTime);
		return $this->getdate("seconds");
	}
	...
	function EOW($dateTime="")
	{
		if ($dateTime) $this->SetDate($DateTime);
		$dt = new DateClass($this->TimeStamp());
		return $dt->Add("day",6-$dt->getdate("wday"));
	}
...
class DateSpanClass
{
	...
	function _calcWeekDays()
	{
		...
		while ($dtStart->TimeStamp() < $dtStop->TimeStamp())
		{
			if ($dtStart->getdate("wday") > 0 && $dtStart->getdate("wday") < 5)
				$counter++;			
			$dtStart->Add("day",1);
		}
