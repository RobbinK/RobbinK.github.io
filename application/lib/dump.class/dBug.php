...
class dbug {
    ...
    var $debugCode = null; // add by Penguin
    var $debugFile = null; // add by Penguin
    var $debugLine = null; // add by Penguin
    var $xmlDepth = array();
    var $xmlCData;
    var $xmlSData;
    var $xmlDData;
    ...
    //add by penguin
    public function setVarname($info) {
        if (is_array($info)) {
            $this->debugCode = $info[0];
            $this->debugFile = $info[1];
            $this->debugLine = $info[2];
        }
    }

    //constructor
    public function debug($var, $forceType = "", $bCollapsed = false) {
    ...
    function makeTableHeader($type, $header, $colspan = 2) {
        if (!$this->bInitialized) {
            if ($this->debugCode) {
                $file = str::summarize($this->debugFile, 45, true, '\\');
                $ln = $this->debugLine;
                $header = "<p style='line-height:18px;font:12px tahoma;'>" . $this->debugCode . " (<b>" . $header . "</b>) ";
                $header.="<br>{$file} (ln:{$ln})";
                $header.= "</p>";
            }
        ...
        echo "<table cellspacing='0' cellpadding='0' class=\"dBug_" . $type . "\">
                <tr>
    ...
    function checkType($var) {
        switch (gettype($var)) {
            ...
            default:
                ...
                $this->makeTableHeader('other', "Other Type");
                $this->makeTDHeader("other", $var);
    ...
    function initJSandCSS() {
        ...
        <style type="text/css">
            ...
            table.dBug_array td { background-color:#FFFFFF;  border: solid 2px #009900; padding: 3px;}
            ...
            /* other */
            table.dBug_other { background-color:#009900; }
            table.dBug_other td { background-color: #78c704; }
            table.dBug_other td.dBug_otherHeader { background-color: #ffdc4d; }
            table.dBug_other td.dBug_otherKey { background-color:#f2f5f7; }            
        </style>
        <?php
...
if (!function_exists('debug')) {
    ...
    function debug($variable) {
        global $debug_funcname;
        $debug_funcname = __function__;
            ...
            function getVarname() {
                ...
                for ($i = count($arrBacktrace) - 1; $i >= 0; $i--) {
                    ...
                    if (array_key_exists("function", $arrCurrent) &&
                            (in_array($arrCurrent["function"], $arrInclude) || (0 != strcasecmp($arrCurrent["function"], $debug_funcname))))
                        continue;
                ...
                if (isset($arrFile)) {
                    ...
                    $debug_pattern = "/\b" . $debug_funcname . "\s*\(\s*(.+)\s*\)" . ";/i";
                    preg_match($debug_pattern, $code, $arrMatches);
                    return @array($arrMatches[1], $arrFile["file"], $arrFile["line"]);
                }
                return "";
            }
