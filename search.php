...
include 'header.php';
include 'menu.php';
include 'admin/answerTickets.php';
class search
{
	var $db;
	...
	public function search_results($s,$userId)
	{
		$baceTicketId = "";
		$subject = "";
		$baceResult = "false";
		$baceMessage = "";
		$department = "";
		$select = mysql_query("SELECT id, userId, subject, msg, status, dateAded, dateActivity,	department FROM  tickets WHERE userId='$userId' AND id LIKE '%$s%' OR subject LIKE '%$s%' OR msg LIKE '%$s%'");
	...
	public function search_answer_result($s,$userId)
	{
		$answeredMessage = "";
		$answeredTicketId = "";
		$answerResult = "false";
		...
		while ($result = mysql_fetch_array($select)) 
		{
			if(self::get_userId($result["ticketId"]) == $userId)
			{
	...
	private function get_userId($ticketId)
	{
		$select = mysql_query("SELECT id,userId FROM  tickets WHERE id='$ticketId'");
		while ($result = mysql_fetch_array($select))
		{
			if($result["id"] == $ticketId)
			{
...
if(isset($_GET["s"]))
{
	$search = new search();
	$s = $_GET["s"];
	$search_array = $search->search_results($s,$userId);
	$search_answer_array = $search->search_answer_result($s,$userId);
		...
		echo '
			<article class="container" style="margin-top:-18px;">
					<section class="well well-small mePadd">
        				<h2 class="meFont" style="padding-bottom:30px;"><span class="icon-comment meMarg"></span> Search result for '.$s.'</h2>';
