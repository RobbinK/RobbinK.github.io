...
class pengu_mysql
{
    ...
    private $result_type;
    private $_tableAlias;
    private $return_array_type = MYSQL_ASSOC; //MYSQL_ASSOC ,MYSQL_BOTH , MYSQL_NUM
    ...
    public function __construct($config)
    {
        ...
        pengu_mysql_trace::$logsavepath = $this->db_logs_path;
    ...
    {
        if (in_array($restype, array(MYSQL_ASSOC, MYSQL_BOTH, MYSQL_NUM)))
    ...
    private function connect()
    {
        ...
        if ($this->host_persist)
            $this->handle = @mysql_pconnect($this->host_name, $this->host_username, $this->host_password);
        else
            $this->handle = @mysql_connect($this->host_name, $this->host_username, $this->host_password);
        ...
        if (!@mysql_select_db($this->host_db, $this->handle)) {
            $this->handle = null;
            return false;
        }
        if (isset($this->db_encoding))
            if (!@mysql_query("SET NAMES '{$this->db_encoding}'", $this->handle)) {
...
//    
    ...
    {
        if ((!isset($this->handle->transactionStatus) || $this->handle->transactionStatus != 'began') && mysql_query("BEGIN", $this->handle)) {
    ...
    {
        if ((isset($this->handle->transactionStatus) && $this->handle->transactionStatus == 'began') && mysql_query("COMMIT", $this->handle)) {
    ...
    {
        if ((isset($this->handle->transactionStatus) && $this->handle->transactionStatus == 'began')  && mysql_query("ROLLBACK", $this->handle)) {
    ...
    public function exec(array $params = null)
    {
        ...
        if (!empty($this->sql)) {
            ...
            if (DEVELOP)
                microtimer::start();
            if ($this->sql_getplace('Qtype') == 'select' || preg_match('/^select\s/i', trim($this->sql)) || preg_match('/^show\s/i', trim($this->sql))) {
                ...
                if ($this->caching) {
                    ...
                    if (!$cached) {
                        // wr: if didn't cache
                        $this->ping();
                        $this->result = mysql_query($this->sql, $this->handle);
                        $asMysqlResult = true;
                    } else {
                        // wr: if cached
                ...
                if (is_bool($result)) {
                    ...
                    if ($this->sql_getplace('Qtype') == 'insert' || $insid = @mysql_insert_id($this->handle))
                        $this->lastinsid = isset($insid) ? $insid : @mysql_insert_id($this->handle);
            ...
            if ($this->result_type == 'wr') {
                $obj = new pengu_mysql_result($this->result, $this->return_array_type, $this->lastsql);
                //if ($asMysqlResult)
                //  $this->free_result();
    ...
    {
        $sqlError = mysql_error($this->handle);
...
class pengu_mysql_result
{
    ...
    private function resultToArray($result, $return_array_type = MYSQL_ASSOC)
    {
        if (is_array($result))
            return $result;
        $result_array = array();
        if (isset($result))
            while ($row = @mysql_fetch_array($result, $return_array_type))
                $result_array[] = $row;
        return $result_array;
    }
    ...
    public function current()
    {
        ...
        if (!is_array($this->currentrow))
            $this->getcurrentrow();
        if (is_array($this->currentrow)) {
            return new pengu_mysql_result_data($this->currentrow);
        }
    }
...
{
    return pengu_mysql:: condition($params, $opration, $safe);
}
