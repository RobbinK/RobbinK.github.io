...
class Router {
    ...
    public function map($routeUrl, $target = '', array $args = array()) {
        ...
        if (isset($args['name'])) {
            if (defined('route_' . $args['name'])) {
                $routeUrl = eval('return route_' . $args['name'] . ';');
                $route->setUrl($this->basePath . $routeUrl);
            }
    ...
    public function match($requestUrl, $requestMethod = 'GET') {
        ...
        $requestUrlnonquery = $requestUrl;
        if (($pos = strpos($requestUrl, '?')) !== false) {
            $requestUrlnonquery = substr($requestUrl, 0, $pos);
        }
        ...
        foreach ($this->routes as $route) {
            ...
            if (strpos($regex, '\\?') !== false) {
                ...
                if (!preg_match("@^" . $regex . "$@i", $requestUrlnonquery, $matches))
                    continue;
            }
            $params = array();
    ...
    public function generate($routeName, array $params = array()) {
        ...
        if ($params && preg_match_all("/(?:\\{)?\\:([\w\-_]+)(?:\\})?/", $url, $param_keys)) {
            ...
            foreach ($param_keys as $i => $key) {
                if (isset($params[$key]))
                    $url = preg_replace("/(?:\\{)?\\:([\w\-_]+)(?:\\})?/", $params[$key], $url, 1);
            }
        }
